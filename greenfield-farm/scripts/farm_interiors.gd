...
func _draw_room_shell(wall: Color, floor_a: Color, floor_b: Color) -> void:
	...
	for y in range(150, 640, 34):
		...
		for x in range(112 - (row%2)*52, 1168, 104):
			var plank_index := int(x / 104) + row
			var plank_color := floor_b if plank_index % 2 == 0 else floor_a.lightened(0.04)
			draw_rect(Rect2(x,y,100,30), plank_color, true)
...
func _draw_store() -> void:
	...
	var crate_colors := [Color("#ece3d0"),Color("#e47733"),Color("#ecc94f")]
	for i in range(3):
		var x := 445 + i*145
		_draw_crate(Vector2(x,405), crate_colors[i])
...
func _draw_plant(pos: Vector2) -> void:
	draw_rect(Rect2(pos.x-20,pos.y,40,34),Color("#9d6040"),true)
