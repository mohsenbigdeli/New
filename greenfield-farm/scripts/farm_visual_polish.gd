...
const LAMPS := [Vector2(980,430),Vector2(1110,820),Vector2(1510,820),Vector2(1900,820)]
...
func _draw_meadow_details() -> void:
	...
	for i in range(18):
		var x := 70.0 + float((i*127)%1950)
		var y := 330.0 + float((i*173)%1030)
		var p := Vector2(x,y)
		for j in range(4):
			var lean := sin(t*1.2+float(i+j))*1.7
			draw_line(p+Vector2(j*4,0),p+Vector2(j*4+lean,-10-j%2*3),Color(0.22,0.45,0.22,0.62),2.0)

func _draw_water_glints() -> void:
	# pond glints
	for i in range(7):
		var a := t*0.35 + float(i)*0.91
		var p := Vector2(1360,560)+Vector2(cos(a)*float(42+i*11),sin(a*1.3)*float(22+i*7))
		var w := 10.0 + float(i%3)*6.0
		draw_line(p-Vector2(w,0),p+Vector2(w,0),Color(0.84,0.97,0.98,0.35),2.0)
	# river travelling highlights
	for i in range(11):
		var y := fmod(t*34.0 + float(i)*143.0,1500.0)
		var x := 2160.0 + sin(float(i)*2.1)*34.0
		draw_line(Vector2(x-18,y),Vector2(x+18,y),Color(0.82,0.96,1.0,0.30),2.0)

func _draw_lamps() -> void:
	var hour := float(farm.time_of_day)/60.0
	var night := hour >= 18.0 or hour < 7.0
	for p in LAMPS:
		...
		if night:
			var pulse := 0.055 + sin(t*2.0+p.x)*0.012
			draw_circle(p+Vector2(0,-45),48,Color(1.0,0.72,0.30,pulse))

func _draw_chimney_smoke() -> void:
	var base := Vector2(150+318,120+23)
	for i in range(5):
		var age := fmod(t*0.28+float(i)*0.19,1.0)
		var p := base + Vector2(sin(age*8.0+float(i))*9.0,-age*82.0)
...
func _draw_npc_portrait(p: Vector2, kind: String, label: String) -> void:
	var bob := sin(t*3.0+p.x*0.01)*1.8
...
func _draw_night_particles() -> void:
	...
	if hour < 18.5 and hour >= 6.5:
		return
	for i in range(16):
		var x := 120.0 + fmod(float(i)*173.0 + sin(t*.7+float(i))*26.0,1900.0)
		var y := 310.0 + fmod(float(i)*109.0 + cos(t*.9+float(i))*18.0,930.0)
		var pulse := 0.35+0.35*sin(t*3.0+float(i))
